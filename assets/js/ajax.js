jQuery(document).ready(function ($) {
    ...
    $('#editwebpagebtn').click(function (e) {
        ...
        var ckeditor_full = $('#ckeditor_full').val();
        $('#hiddendesc').val(ckeditor_full);
...
/**
    ...
    bootbox.confirm("Are you sure?", function (result) {
        if (result == true) {
            ...
            var url = baseurl + 'deletedepartment/' + id;
            $.ajax({
                type: 'GET',
                dataType: 'JSON',
                url: url,
                ...
                success: function (data) {
                    //console.log(data.status);
                    $("#successmsg").html(data.msg).show().delay(3000).fadeOut();
                    $("#viewdepartments").load(window.location + " #viewdepartments");
                },
                error: function (data) {
                    $("#errormsg").html(data.msg + " Failed to Delete").show().delay(3000).fadeOut();
                }
...
/**
    ...
    $('#editdepartmentbtn').click(function (e) {
        ...
        var formData = new FormData($(form)[0]);
        var url = baseurl + 'editdepartmentajax';
        $.ajax({
            type: 'POST',
            dataType: 'JSON',
            url: url,
