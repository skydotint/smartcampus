...
class Debits extends CI_Controller {
    ...
    public function index() {
        ...
        $this->data['tbltitle'] = "View All Debits";
        $this->data['debits'] = $this->accounts_model->getDebits();
      
        $this->load->view('admintheme/header', $this->data);
        $this->load->view('admintheme/sidebar');
        $this->load->view('debits/viewdebits', $this->data);
        $this->load->view('admintheme/footer');
    }

    public function adddebitform() {
        $this->data['user'] = $this->ion_auth->user()->row();
		$this->data['financialdepartments'] = $this->financialdepartments_model->getFinancialdepartments();
        $this->load->view('admintheme/header', $this->data);
        $this->load->view('admintheme/sidebar');
        $this->load->view('debits/adddebitform', $this->data);
        $this->load->view('admintheme/footer');
    }
	
	    public function adddebitajax() {
	...
	public function editdebitform() {
        $this->data['user'] = $this->ion_auth->user()->row();
		$this->id = $this->uri->segment(2);
        $this->where = array('accountid' => $this->id);
        //var_dump($this->where);
		$this->data['debits'] = $this->common_model->getRecords($this->common_model->_accounts, $this->where, 's');
        //var_dump($this->data['debits']);
		$this->data['financialdepartments'] = $this->financialdepartments_model->getFinancialdepartments();
		$this->load->view('admintheme/header', $this->data);
        $this->load->view('admintheme/sidebar');
        $this->load->view('debits/editdebitform', $this->data);
        $this->load->view('admintheme/footer');
    }
	
	    public function editdebitajax() {
	...
	public function deletedebitajax() {
        $this->id = $this->uri->segment(2);
        ...
        if ($this->id) {
           ...
           if ($this->isDelete) {
                $this->status['status'] = 1;
                $this->status['msg'] = "Debit Amount is deleted.";
            } else {
                $this->status['status'] = 0;
